...
def main(pdf_file, qr_code_dir, nb_columns, nb_rows):
	...
	text_height = 0.2 * cm
	height = c._pagesize[1] / nb_rows
	page_height = c._pagesize[1]
	image_width = min([width, height - text_height])
	...
	for page in chunker(images, nb_columns * nb_rows):
		for j, row in enumerate(chunker(page, nb_columns)):
			for i, image in enumerate(row):
				c.drawImage("{}/{}".format(qr_code_dir, image), width * i + width / 2 - image_width / 2, page_height - height * j - image_height, image_width, image_height)
