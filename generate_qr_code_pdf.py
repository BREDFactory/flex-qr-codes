...
def main(pdf_file, qr_code_dir, nb_columns, nb_rows):
	c = canvas.Canvas(pdf_file, bottomup=False)
	...
	for page in chunker(images, nb_columns * nb_rows):
		for j, row in enumerate(chunker(page, nb_columns)):
			for i, image in enumerate(row):
				c.drawImage("{}/{}".format(qr_code_dir, image), width * i, height * j, width, image_height)
				image_name = os.path.splitext(image)[0]
				c.drawString(width * i + width / 2 - (len(image_name) * character_width) / 2, height * j + image_height, image_name)
		c.showPage()
	c.save()
